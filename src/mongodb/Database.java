	...
	public static void main(String[] args) {
		try {
			...
			for (int i = 1; i <= 3900; i++) {
				String names[] = { "Jay", "Jim", "Yom", "Axel", "Billy", "Charlie", "Jax", "Gina", "Paul", "Ringo",
						"Ally", "Nicky", "Cam", "Ari", "Trudie", "Cal", "Carl", "Lady", "Lauren", "Ichabod", "Arthur",
						"Ashley", "Drake", "Kim", "Julio", "Lorraine", "Floyd", "Janet", "Lydia", "Charles", "Pedro",
						"Bradley" };
				Random random = new Random();
				int minDay = (int) LocalDate.of(1940, 1, 1).toEpochDay();
				int maxDay = (int) LocalDate.of(1980, 1, 1).toEpochDay();
				...
				writer.printf("db.clients.insert({name:" + "\"%s" + "\",birthdate:" + "\"%s" + "\"});%n",
						names[new Random().nextInt(names.length)], randomBirthDate);
			}
			...
			for (int i = 1; i <= 100; i++) {
				...
				writer.printf("db.clients.insert({name:" + "\"%s" + "\",birthdate:" + "\"%s" + "\"});%n",
						name + " " + names[new Random().nextInt(names.length)], randomBirthDate);
